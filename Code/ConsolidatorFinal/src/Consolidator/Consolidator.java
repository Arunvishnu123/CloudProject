...
public class Consolidator {
  ...
  public static void main(String[] arg) throws IOException {
    ...
    // calculate the total profit of all store and find the least profit one and
    // most profit one
    ByteArrayOutputStream streamStore = new ByteArrayOutputStream();
    OutputStreamWriter streamWriterStore = new OutputStreamWriter(streamStore,
        StandardCharsets.UTF_8);
    CSVWriter writerStream = new CSVWriter(streamWriterStore, ',',
        Character.MIN_VALUE, '"', System.lineSeparator());
    ...
    for (S3ObjectSummary os1 : objectsStores) {
      S3Object object1 = s3
          .getObject(new GetObjectRequest(bucketName1, os1.getKey()));
      // System.out.println(os1.getKey());

      BufferedReader reader1 = new BufferedReader(
          new InputStreamReader(object1.getObjectContent()));
      String storeTotalProfit = reader1.readLine();
      totalProfitoFStores.put(os1.getKey(),
          Double.parseDouble(storeTotalProfit));
    }
    ...
    for (String key : totalProfitoFStores.keySet()) {
      ...
      allStoresTotalProfit = allStoresTotalProfit
          + totalProfitoFStores.get(key);
    ...
    for (Entry<String, Double> entry : totalProfitoFStores.entrySet()) {
      ...
      if (entry.getValue() == minimumProfit) {
        // System.out.println("The key for value " + minimumProfit + " is " +
        // entry.getKey());
    ...
    System.out.println("Total profit of all store " + allStoresTotalProfit);
    writerStream.writeNext(new String[] {"All Store Total Profit",
        allStoresTotalProfit.toString()});
    writerStream.writeNext(
        new String[] {"Store with maximum profit", storeMaximumProfit});
    System.out.println("Store have maximum profit " + storeMaximumProfit);
    writerStream.writeNext(
        new String[] {"Store with minimum profit", storeMinimumProfit});
    System.out.println("Store with minimum profit " + storeMinimumProfit);
    ...
    HashMap<String,
        List<Product>> products = new HashMap<String, List<Product>>();

    for (S3ObjectSummary os : objects) {
      System.out.println("*" + os.getKey());
      S3Object object = s3
          .getObject(new GetObjectRequest(bucketName, os.getKey()));
    ...
    writerStream.writeNext(new String[] {"ProductName", "ProductTotalProfit",
    ...
    for (String key : products.keySet()) {
      ...
      writerStream.writeNext(
          new String[] {product, String.valueOf(totalProfitAllStores),
              String.valueOf(totalQuantityAllStores)});
      System.out.println(product + ",total quantity - " + totalQuantityAllStores
          + ", total profit" + totalProfitAllStores);
    }
    writerStream.close();
    ...
    try {
      ...
      s3.putObject("finalconsolidatoroutput",
          today.getTime() + "productprofit" + ".csv",
          new ByteArrayInputStream(streamStore.toByteArray()), meta);

    } catch (AmazonServiceException e) {
      System.err.println(e.getErrorMessage());
    }

    // delete the all excel files from the bucket
    ObjectListing productBucket = s3.listObjects(bucketName);
    List<S3ObjectSummary> objectsStores1 = productBucket.getObjectSummaries();

    for (S3ObjectSummary objects2 : objectsStores1) {

      try {
        s3.deleteObject(bucketName, objects2.getKey());
      } catch (AmazonServiceException e) {
        System.err.println(e.getErrorMessage());
        System.exit(1);
      }
    ...
    // delete the all excel files from the bucket
    ObjectListing totalProfitBucket = s3.listObjects(bucketName1);
    List<S3ObjectSummary> objectsStores2 = totalProfitBucket
        .getObjectSummaries();

    for (S3ObjectSummary objects3 : objectsStores2) {

      try {
        s3.deleteObject(bucketName1, objects3.getKey());
      } catch (AmazonServiceException e) {
        System.err.println(e.getErrorMessage());
        System.exit(1);
      }
